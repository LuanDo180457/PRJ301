...
public class CreateVoucherServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (isValidInput(name, discount, expiryDate)) {
            ...
            if (vDAO.create(new Voucher(0, name, discount, expiryDate, true)) == 1) {
    ...
    private boolean isValidInput(String name, int discount, Date expiryDate) {
        return name != null && !name.trim().isEmpty() && discount >= 0 && discount <= 100 && expiryDate != null;
    }
}